...
const updateUserInfo = async (req: Request, res: Response) => {
    try {
        ...
        if (userData.userId === Number(userId)) {
            const result = await userServices.updateUserToDB(Number(userId), validatedData);
            res.status(200).json({
                success: true,
...
const createAnOrder = async (req: Request, res: Response) => {
    try {
        ...
        const validatedData = userOrderValidationSchema.parse(orderData)

        const result = await userServices.addOrdersToUserDB(Number(userId), validatedData);
        res.status(200).json({
            success: true,
